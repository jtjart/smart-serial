...
def _device(fake_transport) -> SmartUX60:
...
async def test_get_value_sends_get_command(fake_transport) -> None:
...
async def test_set_value_sends_set_command(fake_transport) -> None:
    fake_transport.responses["set brightness=65"] = "brightness=65"
    device = _device(fake_transport)
    assert await device.set_value("brightness", "65") == "65"


async def test_set_value_with_source_targets_inactive_source(fake_transport) -> None:
...
async def test_adjust_value_uses_plus_for_positive_delta(fake_transport) -> None:
...
async def test_adjust_value_uses_minus_for_negative_delta(fake_transport) -> None:
...
async def test_power_on(fake_transport) -> None:
...
async def test_power_off_first_stage_returns_confirm_off(fake_transport) -> None:
    fake_transport.responses["off"] = "powerstate=Confirm off"
    device = _device(fake_transport)
    assert await device.power_off() == "Confirm off"


async def test_power_off_now_sends_bare_verb(fake_transport) -> None:
...
async def test_get_power_state_uses_get_powerstate(fake_transport) -> None:
...
async def test_mute_and_unmute(fake_transport) -> None:
...
async def test_set_volume(fake_transport) -> None:
...
async def test_network_enable_disable(fake_transport) -> None:
...
async def test_get_network_status(fake_transport) -> None:
...
async def test_get_lamp_hours(fake_transport) -> None:
    fake_transport.responses["get lamphrs"] = "lamphrs=1234"
    device = _device(fake_transport)
    assert await device.get_lamp_hours() == 1234


async def test_reset_lamp_hours_sends_bare_set_with_no_value_echoed_back(fake_transport) -> None:
...
async def test_restore_defaults_sends_bare_command(fake_transport) -> None:
